defmodule BrodMimic.Brod do
  ...
  - `value` can have many different forms:
    ...
    - `{Brod.msg_ts(), binary()}`: Single message with its create-time timestamp
      and key from `key`
    - `{ts: Brod.msg_ts(), value: binary(), headers: [{_, _}]}`: Single message.
      If this map does not have a `key` field, `key` is used instead.
    - `[{k, v} | {t, k, v}]`: A batch, where `v` could be a nested list of such
      representation
    - `[%{key: k, value: v, ts: t, headers: [{_, _}]}]`: A batch

    When `value` is a batch, the `key` argument is only used as partitioner
    input and all messages are written on the same partition.
    ...
    `ts` field is dropped for Kafka prior to version `0.10` (produce API version
     ...
     `partition` may be either a concrete partition (an integer) or a
     partitioner (see `t:partitioner/0` for more info).

     A producer for the particular topic has to be already started (by calling
     {`start_producer/3`}), unless you have specified `auto_start_producers:
     true` when starting the client.
     ...
     :brod_produce_req_acked}` (see the `t:produce_reply/0` type) message after
     the produce request has been acked by Kafka.
